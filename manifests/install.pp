...
class shibboleth::install {
  ...
  exec { 'bootstrap idp home':
    ...
    environment => [
      "ANT_OPTS=\"
        -Didp.src.dir=/opt/staging/shibboleth-identity-provider-${shibboleth::version}
        -Didp.target.dir=${shibboleth::idp_home}
        -Didp.host.name=${shibboleth::host_name}
        -Didp.noprompt=yes
        -Didp.no.tidy=yes
        -Didp.keystore.password=${shibboleth::keystore_pass}
        -Didp.sealer.password=${shibboleth::sealer_pass}
        -Didp.merge.properties=/opt/staging/shibboleth-identity-provider-${shibboleth::version}/shibboleth.properties \"",
